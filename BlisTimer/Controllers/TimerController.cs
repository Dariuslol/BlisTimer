...
namespace BlisTimer.Controllers
{
    public class TimerController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            //If the user did not select a project yet all projects need to be given to the View otherwise only the project the the user chose has to be given.
            if (String.IsNullOrEmpty(projectId))
            {
                ...
                //the user has selected a project and this project will be saved in a viewbag for later use in the view
                ...
                foreach (var project in projectsOfUser)
                {
                    //We find the project that the user selected and check if the user has already an activity selected.
                    if (project.Id == SelectedProject.Id)
                    {
                        projectDict.Add(Tuple.Create(project, true), project.Activities.ToList());
                        //if the user has an activity selected it will define the viewbags needed and give all the hourtypes in a viewbag. so we know which one to preselect in the dropdown
                        if (!String.IsNullOrEmpty(HttpContext.Session.GetString("ActivityId")))
                        {
                            ...
                            foreach (var hourtype in hourTypeOfUser)
                            {
                                ...
                                {
                                    //If the user has already selected an hourtype we should give back the selected hourtype to the view using viewbag, so we know which one to preselect in the dropdown.
